  ...
  return (
    <main className="min-h-screen bg-[#0a0f1e] text-white">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <p className="text-slate-400 mb-10">Platform-wide statistics and incident trends.</p>

        {/* Summary Cards */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            { label: 'Total Reports', value: reports.length, color: 'text-white' },
            { label: 'Total Users', value: userCount, color: 'text-cyan-400' },
            { label: 'Pending', value: byStatus['pending'] || 0, color: 'text-yellow-400' },
            { label: 'Resolved', value: (byStatus['resolved'] || 0) + (byStatus['closed'] || 0), color: 'text-green-400' },
          ].map((s) => (
        ...
        {reports.length === 0 ? (
          <div className="bg-[#111827] border border-slate-700/50 rounded-xl p-12 text-center">
            <p className="text-slate-400 text-sm">No report data yet.</p>
          </div>
        ) : (
